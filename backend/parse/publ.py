...
def sampleInproceedings(context, output_file, n_samples = -1):
    ...
    with open(output_file, 'w+') as f:
        ...
        for event, elem in context:
            if event == 'end' and elem.tag == 'inproceedings' and elem.attrib.get('publtype') is None:
...
if __name__ == '__main__':
    ...
    context = etree.iterparse(f'{PATH_DATA}/dblp.xml', events = ('end',), load_dtd = True)

    sampleInproceedings(context, f'{PATH_DATA}/inproceedings.xml', 10)
    exit(0)
