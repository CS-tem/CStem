...
# assumed a '-' separated string will be given here
@app.get('/articles/selected-topics/{topic_id}')
def get_articles_by_topic(topic_id: str, q: Optional[str] = None):
    if q:
        topics = q
...
# assumed a '-' separated string will be given here
@app.get('/institutes/selected-countries/{cnt_str}')
def get_institutes_by_country(cnt_str: str, q: Optional[str] = None):
    if q:
        countries = q
...
# assumed a '-' separated string will be given here
@app.get('/venues/selected-types/{type_string}')
def get_venues_by_type(type_string: str, q: Optional[str] = None):
    if q:
        venues = q
...
def post_new_authors_info(request: NewAuthorsCondition):
    query1 = """MATCH (i: Author)-[:AuthorArticle]->(j)<-[:ArticleTopic]-(x)
    WHERE j.year >= {} AND j.year <= {} AND x.name in {}
    RETURN i, COALESCE(count(DISTINCT j),0) AS n_pubs""".format(
        request.frm, request.to, request.topics
    )
    query2 = """
    CALL{{WITH *  MATCH (a: Author)-[:AuthorArticle]->(b)<-[:ArticleTopic]-(c)
    OPTIONAL MATCH (b)-[:CitedBy]->(k)
    WHERE b.year >= {} AND b.year <= {} AND k.year >= {} AND k.year <= {}
    AND c.name IN {}
    RETURN a,b,COALESCE(COUNT(DISTINCT k),0) AS n_cits}}
    RETURN a as i, SUM(n_cits) AS n_citations ;
    """.format(
        request.frm, request.to, request.frm, request.to, request.topics
    )
